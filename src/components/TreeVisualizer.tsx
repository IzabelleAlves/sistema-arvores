...
export const TreeVisualizer: React.FC<TreeVisualizerProps> = ({ data, type }) => {
  ...
  useEffect(() => {
    ...
    // 1. Invertemos o size: a "largura" do layout vira a altura do SVG e vice-versa
    const treeLayout = d3
    ...
    const svg = d3.select(svgRef.current)
      ...
      .attr("height", height)
      .append("g")
      .attr("transform", "translate(80, 20)"); // Margem maior na esquerda para o texto inicial

    // 2. Mudamos para linkHorizontal e invertemos x por y
    svg.selectAll('path')
      .data(root.links())
      .enter()
      .append('path')
    ...
    // 3. Invertemos a translação dos nós: (d.y, d.x)
    ...
    // 4. Ajustamos o texto para as laterais
    nodes.append('text')
      .attr('dy', '0.31em')
      .attr('x', (d: any) => d.children ? -12 : 12) // Texto à esquerda se tiver filhos, à direita se for folha
